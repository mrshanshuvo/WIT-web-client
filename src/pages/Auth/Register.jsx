...
const Register = () => {
    ...
    const checkEmail = async () => {
      ...
      try {
        const { data } = await axiosInstance.get(
          `/users/check-email?email=${emailValue}`
        );
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const { user } = await createUser(emailValue, password);
      await updateUserProfile({
        displayName: name,
        photoURL: photoURL || null,
      });

      // Unified Backend Sync (this will handle setUser as well)
      await handleBackendAuth(user, { name, photoURL });
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full pl-3.5 pr-11 py-2.5 border rounded-lg focus:ring-2 transition-all duration-150 bg-white/60 backdrop-blur-sm text-sm ${emailStatus === "taken" || emailStatus === "invalid"
                  ...
                  <div className="mt-1.5 h-4">
                    {emailStatus === "checking" && (
                      <p className="text-[11px] text-gray-500 flex items-center gap-1.5">
                        <span className="w-2 h-2 border-t-2 border-emerald-500 rounded-full animate-spin" />
                ...
                <div>
                  ...
                  <div className="mt-3 p-3.5 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border border-emerald-200">
                    ...
                    <div className="grid grid-cols-2 gap-1.5 text-xs sm:text-sm">
                      <div
                        className={`flex items-center gap-2 ${passwordCriteria.length
                          ? "text-emerald-600"
                          : "text-gray-500"
                          }`}
                      >
                      ...
                      <div
                        className={`flex items-center gap-2 ${passwordCriteria.uppercase
                          ? "text-emerald-600"
                          : "text-gray-500"
                          }`}
                      >
                      ...
                      <div
                        className={`flex items-center gap-2 ${passwordCriteria.lowercase
                          ? "text-emerald-600"
                          : "text-gray-500"
                          }`}
                      >
                      ...
                      <div
                        className={`flex items-center gap-2 ${passwordCriteria.number
                          ? "text-emerald-600"
                          : "text-gray-500"
                          }`}
                      >
